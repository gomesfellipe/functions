#Fonte da funcao
#http://www.sthda.com/english/wiki/correlation-matrix-an-r-function-to-do-all-you-need#at_pco=smlre-1.0&at_si=5a53f0ab57c3d5c4&at_ab=per-2&at_pos=2&at_tot=4
                        ...
                        col=NULL, method = "pearson", ...)
{
  ...
  cor.pmat <- function(x, ...) {
    ...
    for (i in 1:(n - 1)) {
      for (j in (i + 1):n) {
        tmp <- cor.test(mat[, i], mat[, j], method = as.character(method))
...
cormat(mydata,type = "lower",method = "spearman")
